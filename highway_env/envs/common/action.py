...
from highway_env.typing import ActionDict, Vector
...
class ActionType:
    ...
    def __init__(self, env: AbstractEnv, **kwargs) -> None:
        ...
        self.__controlled_vehicle: MDPVehicle | None = None
    ...
    def controlled_vehicle(self, vehicle: MDPVehicle):
...
class ContinuousAction(ActionType):
    ...
    def get_action(self, action: Vector) -> ActionDict:
        if self.clip:
        ...
        # @minokori: use match-case when Python 3.10+ is required, for better readability
        # and add an False, False case for better error handling.
        match self.longitudinal, self.lateral:
            case True, True:
                return {
    ...
    def act(self, action: Vector) -> None:
...
class DiscreteAction(ContinuousAction):
    ...
    def act(self, action: int) -> None:
        ...
        all_actions: list[tuple[np.float64, ...]] = list(itertools.product(*axes))
        super().act(np.array(all_actions[action]))
...
class DiscreteMetaAction(ActionType):
        ...
        **kwargs,
        ...
        self.actions: dict[int, str] = (
            ...
            if longitudinal and lateral
                ...
                self.ACTIONS_LONGI
                if longitudinal
                else self.ACTIONS_LAT if lateral else {}
            )
        )
        if not self.actions:
            raise ValueError(
    ...
    @property
    def vehicle_class(self) -> functools.partial[MDPVehicle]:
        return functools.partial(MDPVehicle, target_speeds=self.target_speeds)

    def act(self, action: int | Vector) -> None:
